...
export default function NavLink({to, name, icon, arrow = true}: NavLinkProps) {
    ...
    return (
        <div className={`px-4 py-2 hover:bg-skyWave bg-skyWave ${active ? 'bg-skyWave' : 'bg-transparent'}`}>
            <Link to={to}
                  className='flex justify-between items-center no-underline text-snow'>
